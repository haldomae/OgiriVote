...
- [ ] 大会終了後のターン結果(得票)エクスポート要否・方法(アンケート結果のCSVエクスポートは10章で実装済み)

---

## 10. アンケート機能

大会終了後、来場者に任意のアンケートに回答してもらうための機能。**大会(1回のイベント)には紐付けない**運用とし、設問自体は使い回し、回答結果だけをイベント後にまとめてリセットできるようにする。

### 10.1 データモデル

`tournament/state` に以下のフィールドを追加する。

```jsonc
{
  "status": "not_started" | "in_progress" | "finished",
  "currentTurnId": "turn_01" | null,
  "surveyOpen": false   // true の間だけ投票ページがアンケート画面を表示する
}
```

新規コレクション `surveyQuestions/{questionId}`(設問マスタ。大会をまたいで使い回す):
...
```jsonc
{
  ...
  "type": "choice" | "text",
  "question": "設問文",
  "choices": ["選択肢1", "選択肢2"]   // type が "choice" の場合のみ使用
}
```

新規コレクション `surveyResponses/{responseId}`(1回答＝1ドキュメント。個々の投票記録は保持しない投票機能とは異なり、自由記述回答は集計できないため個別に保存する):

```jsonc
{
  "submittedAt": "<serverTimestamp>",
  "answers": {
    "<questionId>": "選択された選択肢の文字列、または記入された自由記述テキスト"
  }
}
```
...
- `tournament/state` の `status` が `finished` かつ `surveyOpen` が `true` になったことを検知すると、**リロードなしで自動的に**アンケート回答画面へ切り替わる(既存の `onSnapshot` 購読の延長で実現。5.5節の方針と同様、購読対象は `tournament/state` のみで、アンケートの設問一覧はこの遷移が起きたタイミングで単発の `getDocs` により取得する)
- 画面デザインは投票画面(6.1節)と統一する: 赤系の背景、黒枠+ハードシャドウの「選択肢タイル」による単一選択式(選択肢タイルは投票画面の参加者カードと同じ見た目・配色を流用し、写真部分を省いたテキストのみのタイルとする)、記入式の設問は黒枠のテキストエリアで表示する
- 全設問に回答するまで画面下部の「送信する」ボタンは非活性のまま固定表示する。送信で `surveyResponses` に1件作成し、完了後は静的な完了メッセージを表示する
- 送信は投票と同様に匿名の一方向書き込み(`addDoc` による新規作成のみ)。個々のクライアントは同一ページセッション内で一度送信すると、以後は完了メッセージが表示され続ける(再送信の抑止は行うが、多重投票対策と同様に厳密な不正対策ではない)

### 10.3 管理者ページの挙動

- 「大会全体の制御」パネルに「アンケートを送信」ボタンを追加する。大会が `finished` の場合のみ有効化し、押すと `tournament/state.surveyOpen` を `true` にする。「アンケートを終了」ボタンで `false` に戻せる
- アンケート設問の登録・編集・削除(`surveyQuestions`)。設問は「選択肢」(2択以上の選択肢を持つ)と「記入欄」(自由記述)の2種類
- アンケート結果の閲覧(`surveyResponses`)。選択肢設問は得票結果と同様のバーチャートで集計表示し、記入欄設問は回答の一覧を表示する
- **CSVエクスポート**: 全回答を「回答日時 + 設問ごとの回答」の表形式でCSVファイルとしてダウンロードできる(サーバー処理を介さず、取得済みの回答データからブラウザ内でCSV文字列を生成してダウンロードさせる)
- **結果の一括リセット**: `surveyResponses` の全ドキュメントを削除するボタンを用意する。設問定義(`surveyQuestions`)自体は削除されず、次回大会でもそのまま再利用できる

### 10.4 セキュリティルール追加分(7章の方針に準拠)

- `surveyQuestions`: 読み取りは誰でも可、書き込みは認証済み管理者のみ
- `surveyResponses`: 新規作成(`create`)は誰でも可(投票ページからの匿名送信のため)。読み取り・削除は認証済み管理者のみ。更新は一切禁止(送信後の改ざん防止)