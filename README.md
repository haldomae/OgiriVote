...
大会終了後、来場者にアンケートを配信できる。設問は大会(1回のイベント)には紐付かず使い回す想定で、結果だけをイベントごとにリセットする運用。

1. 事前に「アンケート設問管理」で設問を登録(種類は「選択肢」または「記入欄」)
2. 大会が「大会終了」の状態になったら、「大会全体の制御」パネルの「アンケートを送信」を押す → 投票ページがリロードなしで自動的にアンケート画面に切り替わる
3. 来場者はカード選択(選択肢)・テキスト入力(記入欄)に回答し、「送信する」で送信完了
4. 「アンケート結果」パネルでリアルタイムに集計を確認(選択肢はバーチャート、記入欄は回答一覧)
5. 「CSVエクスポート」で全回答をダウンロード
6. イベント終了後、「結果をリセット」で回答をすべて削除(設問定義は残るので次回もそのまま使える)
7. 必要に応じて「アンケートを終了」でアンケート配信を停止できる
